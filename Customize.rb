...
version = "0.1.0"
# Color scheme
# Prefix the hex by a hashcode!
primary_color = "#cd7a82"
secondary_color = "#31c9d9"
...
system("clear") || system("cls") || system("Clear-Host")
...
if reply == "y"
  ...
  checks = [
    ...
    ["project's PHP namespace", namespace],
    ...
    ["project's primary color", primary_color],
    ["project's secondary color", secondary_color]
  ]
  ...
  steps = [
    ["{{PKG_NAME}}", name],
    ["{{PKG_REPO}}", slug],
    ["{{PKG_REPO_UNDERSCORE}}", slug.gsub(/['`´- ]/, "_")],
    ["{{PKG_NAMESPACE}}", namespace],
    ["{{PKG_NAMESPACE_DOUBLE}}", namespace.gsub(/\\/, "\\\\")],
    ["{{PKG_AUTHOR}}", author],
    ["{{PKG_USERNAME}}", github],
    ["{{PKG_USEREMAIL}}", email],
    ["{{PKG_USERURI}}", site],
    ...
    ["{{PKG_PRIM_COLOR}}", primary_color],
    ["{{PKG_SEC_COLOR}}", secondary_color]
  ]
